...
def test_calculate_dry_spell_fraction(daily_gsdr_data, daily_gpcc_data):
    result = data_utils.calculate_dry_spell_fraction(
        daily_gsdr_data[DEFAULT_RAIN_COL], target_gauge_col=DEFAULT_RAIN_COL, dry_period_days=15
    )
...
def test_check_data_has_consistent_time_step(hourly_gsdr_data, inconsistent_timestep_data):
    data_utils.check_data_has_consistent_time_step(hourly_gsdr_data)
    with pytest.raises(ValueError):
        data_utils.check_data_has_consistent_time_step(inconsistent_timestep_data)


def test_check_data_is_specific_time_res(hourly_gsdr_data):
    data_utils.check_data_is_specific_time_res(hourly_gsdr_data, time_res="1h")
    with pytest.raises(TypeError):
        data_utils.check_data_is_specific_time_res(hourly_gsdr_data, time_res=120)
    with pytest.raises(ValueError):
        data_utils.check_data_is_specific_time_res(hourly_gsdr_data, time_res="1d")


def test_check_for_negative_values(daily_gsdr_data):
    result = data_utils.check_for_negative_values(daily_gsdr_data, target_gauge_col=DEFAULT_RAIN_COL)
    assert result is False
    daily_gsdr_data_with_neg = daily_gsdr_data.with_columns(pl.col(DEFAULT_RAIN_COL) * -1)
    result = data_utils.check_for_negative_values(daily_gsdr_data_with_neg, target_gauge_col=DEFAULT_RAIN_COL)
...
def test_convert_daily_data_to_monthly(daily_gsdr_data, daily_gpcc_data, hourly_gsdr_data):
    ...
    with pytest.raises(ValueError):
        data_utils.convert_daily_data_to_monthly(hourly_gsdr_data, rain_cols=[DEFAULT_RAIN_COL])


def test_check_data_is_monthly(hourly_gsdr_data, monthly_gsdr_network, monthly_gpcc_data):
    data_utils.check_data_is_monthly(monthly_gpcc_data)
    data_utils.check_data_is_monthly(monthly_gsdr_network)
    with pytest.raises(ValueError):
...
def test_get_data_timesteps(hourly_gsdr_data, inconsistent_timestep_data):
    result = data_utils.get_data_timesteps(hourly_gsdr_data)
    assert len(result) == 1
...
def test_get_data_timestep_as_str(hourly_gsdr_data, inconsistent_timestep_data):
    result = data_utils.get_data_timestep_as_str(hourly_gsdr_data)
    assert result == "1h"
    with pytest.raises(ValueError):
        data_utils.get_data_timestep_as_str(inconsistent_timestep_data)


def test_normalise_data(hourly_gsdr_data):
    result = data_utils.normalise_data(hourly_gsdr_data[DEFAULT_RAIN_COL])
...
def test_offset_data_by_time(hourly_gsdr_data):
    result = data_utils.offset_data_by_time(
        hourly_gsdr_data, target_col=DEFAULT_RAIN_COL, offset_in_time=1, time_res="hourly"
    )
    assert result["rain_mm"][1] == 0.9
    assert result["rain_mm"][2] == 0.3
    result = data_utils.offset_data_by_time(
        hourly_gsdr_data, target_col=DEFAULT_RAIN_COL, offset_in_time=2, time_res="hourly"
    )
    ...
    result = data_utils.offset_data_by_time(
        hourly_gsdr_data, target_col=DEFAULT_RAIN_COL, offset_in_time=0, time_res="hourly"
    )
