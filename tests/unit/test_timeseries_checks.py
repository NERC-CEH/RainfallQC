...
def test_dry_period_cdd_check_daily_gpcc(daily_gpcc_data, gsdr_metadata):
    ...
    assert len(result.filter(pl.col("dry_spell_flag").fill_null(0.0) > 0)) == 0
...
def test_monthly_accumulations_daily_data(daily_gsdr_data, gsdr_metadata):
    ...
    assert len(result.filter(pl.col("monthly_accumulation").fill_null(0.0) > 0)) == 93
...
def test_flag_streaks_of_zero_bounded_by_days(hourly_gsdr_data, min15_gsdr_data_streaky):
    ...
    assert len(result.filter(pl.col("streak_flag5").fill_null(0.0) > 0)) == 120
    ...
    assert len(result.filter(pl.col("streak_flag5").fill_null(0.0) > 0)) == 480
...
def test_flag_streaks_exceeding_smallest_measurable_rainfall_amount(hourly_gsdr_data, gsdr_metadata):
    ...
    assert len(result.filter(pl.col("streak_flag3").fill_null(0.0) > 0)) == 455
    ...
    assert len(result.filter(pl.col("streak_flag3").fill_null(0.0) > 0)) == 288
...
def test_flag_streaks_exceeding_wet_day_rainfall_threshold(hourly_gsdr_data, gsdr_metadata):
    ...
    assert len(result.filter(pl.col("streak_flag1").fill_null(0.0) > 0)) == 23
    ...
    assert len(result.filter(pl.col("streak_flag1").fill_null(0.0) > 0)) == 71
