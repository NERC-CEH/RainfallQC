...
def test_check_exceedance_of_UK_1hr_record(min15_gsdr_data):
    ...
    assert len(result.filter(pl.col("UK_1hr_record_flag").fill_null(0.0) > 0)) == 4420
    assert len(result.filter(pl.col("UK_1hr_record_flag").fill_null(0.0) == 0)) == 170873
...
def test_check_exceedance_of_UK_1h_record_1min_data(min1_gsdr_data):
    ...
    assert len(result.filter(pl.col("UK_1hr_record_flag").fill_null(0.0) > 0)) == 66300
